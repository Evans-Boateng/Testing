...
export default function DeleteNote({open, onClose, noteId}) {
  ...
  return (
    <motion.div
      ...
      onClick={onClose} className={`fixed z-50 inset-0 flex justify-center items-center transition-colors ${open ? "visible bg-black/20" : "hidden"}`}
    >
      <div onClick={(event) => event.stopPropagation()} className={`sm:w-[35%] w-[70%] flex flex-col justify-center items-center px-[15px] py-[20px] rounded-[5px] bg-white shadow transition all relative ${open ? "scale-100 opacity-100" : "scale-125 opacity-0"} `}>
